...
try {
    ...
    $stmt = $conn->prepare("
        SELECT s.id, s.first_name, s.last_name, s.gender, s.schedule_id,
    ...
    // Group students per schedule for quick access in the UI
    $schedule_students = [];
    foreach ($all_students as $student) {
        if (!empty($student['schedule_id'])) {
            $schedule_students[$student['schedule_id']][] = [
                'id' => $student['id'],
                'name' => $student['first_name'] . ' ' . $student['last_name'],
                'parent' => $student['parent_first_name'] . ' ' . $student['parent_last_name'],
                'parent_email' => $student['parent_email'],
                'time_range' => ($student['start_time'] && $student['end_time'])
...
<body>
                                ...
                                <div class="schedule-capacity mb-3">
                                    ...
                                    <div class="progress mt-2">
                                        <div class="progress-bar <?php echo $schedule['enrolled_count'] >= $schedule['max_capacity'] ? 'bg-danger' : 'bg-success'; ?>" 
                                             style="width: <?php echo $schedule['max_capacity'] > 0 ? min(100, ($schedule['enrolled_count'] / $schedule['max_capacity']) * 100) : 0; ?>%"></div>
                                    </div>
                                </div>
                                <button 
                                    class="btn btn-outline-primary btn-sm w-100"
                                    onclick="viewScheduleStudents(this)"
                                    data-schedule-name="<?php echo htmlspecialchars($schedule['schedule_name']); ?>"
                                    data-students="<?php echo htmlspecialchars(json_encode($schedule_students[$schedule['id']] ?? []), ENT_QUOTES, 'UTF-8'); ?>">
    ...
    <!-- Schedule Students Modal -->
    <div class="modal fade" id="scheduleStudentsModal" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Students in Schedule</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div id="scheduleStudentsContent">
    ...
    <script>
        ...
        function viewScheduleStudents(button) {
            const scheduleName = button.getAttribute('data-schedule-name') || 'Schedule';
            const students = JSON.parse(button.getAttribute('data-students') || '[]');
            const modalTitle = document.querySelector('#scheduleStudentsModal .modal-title');
            const contentContainer = document.getElementById('scheduleStudentsContent');

            modalTitle.textContent = `${scheduleName} - Assigned Students`;

            if (!students.length) {
                contentContainer.innerHTML = `
                    <div class="text-center py-4">
                        <i class="fas fa-user-times fa-3x text-muted mb-3"></i>
                        <p class="text-muted mb-0">No students assigned to this schedule yet.</p>
                    </div>
                `;
            } else {
                const listItems = students.map(student => `
                    <li class="list-group-item d-flex justify-content-between align-items-start">
                        <div class="me-3">
                            ...
                            <small class="text-muted">Parent: ${student.parent}</small><br>
                            ${student.parent_email ? `<small class="text-muted">${student.parent_email}</small>` : ''}
                        </div>
                        ${student.time_range ? `<span class="badge bg-light text-dark">${student.time_range}</span>` : ''}
                    </li>
                `).join('');
                ...
                contentContainer.innerHTML = `
                    ...
                    <ul class="list-group schedule-students-list">
                        ${listItems}
                    </ul>
                `;
            }

            new bootstrap.Modal(document.getElementById('scheduleStudentsModal')).show();
        }
    ...
    <style>
        ...
        .schedule-students-list .list-group-item {
            border-left: 4px solid #FF6B9D;
        }
    </style>
