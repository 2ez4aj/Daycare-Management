        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="viewReportTitle">Progress Report</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="row mb-4">
                        <div class="col-md-6">
                            <p><strong>Report Date:</strong> <span id="viewReportDate"></span></p>
                            <p><strong>Student:</strong> <span id="viewStudentName"></span></p>
                            <p><strong>Teacher:</strong> <span id="viewTeacherName"></span></p>
                        </div>
                        <div class="col-md-6 text-md-end">
                            <p><strong>Overall Rating:</strong> <span id="viewOverallRating" class="badge rating-badge"></span></p>
                        </div>
                    </div>
                    ...
                    <div class="card mb-3">
                        ...
                        <div class="card-body">
                            <p id="viewAcademicPerformance"></p>
                        </div>
                    </div>
                    
                    <div class="card mb-3">
                        <div class="card-header">
                            <h6 class="mb-0">Social Skills</h6>
                        </div>
                        <div class="card-body">
                            <p id="viewSocialSkills"></p>
                        </div>
                    </div>
                    
                    <div class="card mb-3">
                        <div class="card-header">
                            <h6 class="mb-0">Physical Development</h6>
                        </div>
                        <div class="card-body">
                            <p id="viewPhysicalDevelopment"></p>
                        </div>
                    </div>
                    
                    <div class="card mb-3">
                        <div class="card-header">
                            <h6 class="mb-0">Behavioral Notes</h6>
                        </div>
                        <div class="card-body">
                            <p id="viewBehavioralNotes" class="mb-0"></p>
                        </div>
                    </div>
                    
                    <div class="card">
                        <div class="card-header">
                            <h6 class="mb-0">Teacher's Comments</h6>
                        </div>
                        <div class="card-body">
                            <p id="viewTeacherComments" class="mb-0"></p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="modal-body">
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <select class="form-select" id="editStudentId" name="student_id" required>
                            ...
                            <div class="col-md-6">
                                <label for="editReportDate" class="form-label">Report Date</label>
                        ...
                        <div class="row mb-3">
                            <div class="col-12">
                                <label for="editAcademicPerformance" class="form-label">Academic Performance</label>
                                <textarea class="form-control" id="editAcademicPerformance" name="academic_performance" rows="3" required></textarea>
                            </div>
                        </div>
                        
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label for="editSocialSkills" class="form-label">Social Skills</label>
                                <textarea class="form-control" id="editSocialSkills" name="social_skills" rows="3" required></textarea>
                            </div>
                            <div class="col-md-6">
                                <label for="editPhysicalDevelopment" class="form-label">Physical Development</label>
                                <textarea class="form-control" id="editPhysicalDevelopment" name="physical_development" rows="3" required></textarea>
                            </div>
                        </div>
                        
                        <div class="row mb-3">
                            <div class="col-12">
                                <label for="editBehavioralNotes" class="form-label">Behavioral Notes</label>
                                <textarea class="form-control" id="editBehavioralNotes" name="behavioral_notes" rows="2"></textarea>
                            </div>
                        </div>
                        
                        <div class="row">
                            <div class="col-md-8">
                                <label for="editTeacherComments" class="form-label">Teacher's Comments</label>
                                <textarea class="form-control" id="editTeacherComments" name="teacher_comments" rows="3"></textarea>
                            </div>
                            <div class="col-md-4">
                                <label for="editOverallRating" class="form-label">Overall Rating</label>
                                <select class="form-select" id="editOverallRating" name="overall_rating" required>
        ...
        function viewReport(reportId) {
            ...
            fetch(`get_report_details.php?id=${reportId}`)
                ...
                .then(report => {
                    ...
                    document.getElementById('viewReportTitle').textContent = `Progress Report - ${report.student_name}`;
                    document.getElementById('viewReportDate').textContent = new Date(report.report_date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
                    document.getElementById('viewStudentName').textContent = report.student_name;
                    document.getElementById('viewTeacherName').textContent = report.teacher_name;
                    document.getElementById('viewAcademicPerformance').textContent = report.academic_performance;
                    document.getElementById('viewSocialSkills').textContent = report.social_skills;
                    document.getElementById('viewPhysicalDevelopment').textContent = report.physical_development;
                    document.getElementById('viewBehavioralNotes').textContent = report.behavioral_notes || 'No notes provided';
                    document.getElementById('viewTeacherComments').textContent = report.teacher_comments || 'No comments provided';
                    
                    // Set overall rating badge
                    const ratingBadge = document.getElementById('viewOverallRating');
                    ratingBadge.textContent = report.overall_rating.charAt(0).toUpperCase() + report.overall_rating.slice(1).replace('_', ' ');
                    ratingBadge.className = 'badge rating-badge ' + `rating-${report.overall_rating}`;
        ...
        function editReport(reportId) {
            ...
            fetch(`get_report_details.php?id=${reportId}`)
                ...
                .then(report => {
                    ...
                    document.getElementById('editStudentId').value = report.student_id;
                    document.getElementById('editReportDate').value = report.report_date.split(' ')[0]; // Format date for input[type=date]
                    document.getElementById('editAcademicPerformance').value = report.academic_performance || '';
                    document.getElementById('editSocialSkills').value = report.social_skills || '';
                    document.getElementById('editPhysicalDevelopment').value = report.physical_development || '';
                    document.getElementById('editBehavioralNotes').value = report.behavioral_notes || '';
                    document.getElementById('editTeacherComments').value = report.teacher_comments || '';
                    document.getElementById('editOverallRating').value = report.overall_rating || 'good';
