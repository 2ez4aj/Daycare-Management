...
if (isset($_GET['get_students'])) {
    try {
        ...
        if (isset($_GET['schedule_id']) && $_GET['schedule_id']) {
            // Get students for specific schedule using schedule_id column
            $stmt = $conn->prepare(
                "SELECT id, first_name, last_name FROM students " .
                "WHERE status IN ('active', 'pending') AND schedule_id = ? " .
            ...
            $stmt = $conn->prepare("SELECT id, first_name, last_name FROM students WHERE status IN ('active', 'pending') ORDER BY first_name, last_name");
            $stmt->execute();
            $students = $stmt->fetchAll(PDO::FETCH_ASSOC);
        }
...
<body>
                    ...
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex gap-2">
                            <form method="GET" class="d-flex gap-2">
                                ...
                                <select name="schedule" class="form-select" style="width: 200px;">
                                    <option value="all" <?php echo (empty($_GET['schedule']) || $_GET['schedule'] === 'all') ? 'selected' : ''; ?>>All Schedules</option>
                                    <?php 
                                    $schedules = $conn->query("SELECT id, schedule_name, start_time, end_time FROM schedules WHERE is_active = 1 ORDER BY start_time")->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($schedules as $sched): 
                                    ?>
                                        <option value="<?php echo $sched['id']; ?>" <?php echo (isset($_GET['schedule']) && $_GET['schedule'] == $sched['id']) ? 'selected' : ''; ?>>
                                            <?php echo htmlspecialchars($sched['schedule_name'] . ' (' . date('g:i A', strtotime($sched['start_time'])) . ' - ' . date('g:i A', strtotime($sched['end_time'])) . ')'); ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                                ...
                                <?php if (isset($_GET['search']) || isset($_GET['schedule'])): ?>
    ...
    <script>
        ...
        document.getElementById('schedule_id').addEventListener('change', loadStudents);
        ...
        markModal.addEventListener('shown.bs.modal', () => {
            loadStudents();
        });

        // Auto-set current time when status is selected
        document.getElementById('status').addEventListener('change', function() {
            // Get current time in local timezone and format as HH:MM
            const now = new Date();
            // Convert to local time in Asia/Manila (UTC+8)
            const options = { timeZone: 'Asia/Manila', hour12: false };
            const currentTime = now.toLocaleTimeString('en-US', { ...options, hour: '2-digit', minute: '2-digit' });
