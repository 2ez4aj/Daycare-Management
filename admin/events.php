    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'add_event':
                ...
                $stmt = $conn->prepare("INSERT INTO events (title, description, event_date, start_time, end_time, location, schedule_id, status, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, 'upcoming', ?, NOW())");
                $stmt->execute([
                    $_POST['title'],
                    $_POST['description'],
                    $_POST['event_date'],
            ...
            case 'edit_event':
                $schedule_id = !empty($_POST['schedule_id']) ? $_POST['schedule_id'] : null;
                $stmt = $conn->prepare("UPDATE events SET title = ?, description = ?, event_date = ?, start_time = ?, end_time = ?, location = ?, schedule_id = ? WHERE id = ?");
                $stmt->execute([
                    $_POST['title'],
                    $_POST['description'],
                    $_POST['event_date'],
...
try {
    ...
    // Get all events
    $stmt = $conn->prepare("SELECT * FROM events ORDER BY event_date DESC, start_time DESC");
    $stmt->execute();
